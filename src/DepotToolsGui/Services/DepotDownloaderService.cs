    ...
    ILogger<DepotDownloaderService> log)
{
    ...
    public IReadOnlyDictionary<long, string> ResolveKeys(long appId)
    {
        ...
        try
        {
            if (steam.EffectivePath is { } root)
            {
                ...
                if (File.Exists(vdf))
                {
                    foreach (var (depot, key) in ExtractConfigVdfKeys(File.ReadAllText(vdf)))
        ...
        IProgress<string>? createdFile = null)
    {
        ...
        try
        {
            ...
            // The manifest id is resolved by the run loop (from the owning app for a shared depot).
            // A local cached .manifest is optional: when absent we omit -manifestfile and the tool
            // fetches the manifest from the CDN itself given -manifest.
            ArgumentNullException.ThrowIfNull(sel.ManifestId);

            psi.ArgumentList.Add("-app"); psi.ArgumentList.Add(appId.ToString());
            psi.ArgumentList.Add("-depot"); psi.ArgumentList.Add(sel.DepotId.ToString());
            psi.ArgumentList.Add("-manifest"); psi.ArgumentList.Add(sel.ManifestId!);
            psi.ArgumentList.Add("-depotkeys"); psi.ArgumentList.Add(keysFile);
            if (sel.ManifestPath is not null)
            {
                psi.ArgumentList.Add("-manifestfile"); psi.ArgumentList.Add(sel.ManifestPath);
            }
            psi.ArgumentList.Add("-dir"); psi.ArgumentList.Add(outDir);
            psi.ArgumentList.Add("-max-downloads"); psi.ArgumentList.Add(MaxChunkDownloads.ToString());
    ...
    // ── config.vdf depot-key extraction (ported from LuaTools' DonateKeysService) ──
    [GeneratedRegex(@"^\d{1,10}$")]
    private static partial Regex ConfigAppIdRegex();
    [GeneratedRegex(@"^[a-zA-Z0-9]{64}$")]
    private static partial Regex ConfigKeyRegex();

    /// <summary>Depot-id → decryption-key pairs from Steam's config/config.vdf, for depots the lua
    /// doesn't carry. Only validated pairs are returned.</summary>
    private static List<(string DepotId, string Key)> ExtractConfigVdfKeys(string content)
    {
        var result = new List<(string, string)>();
        Walk(ParseVdf(content));
        return result;
        ...
        void Walk(Dictionary<string, object> node)
        {
            foreach (var (k, v) in node)
            {
                ...
                if (child.TryGetValue("DecryptionKey", out var dk) && dk is string key
                    && ConfigAppIdRegex().IsMatch(k) && ConfigKeyRegex().IsMatch(key))
                    result.Add((k, key));
                else
    ...
    /// <summary>Minimal recursive VDF (Valve KeyValues) reader: "quoted" tokens, { } nesting, // line
    /// comments. Nested objects → Dictionary; leaf values → string.</summary>
    private static Dictionary<string, object> ParseVdf(string content)
    {
        ...
        while (pos < len)
        {
            ...
            else if (c == '"')
            {
                ...
                else { stack.Peek()[pendingKey] = token; pendingKey = null; }
            }
            else if (c == '{')
            {
