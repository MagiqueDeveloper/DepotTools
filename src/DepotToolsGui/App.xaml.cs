...
public partial class App : Application
{
    ...
    private async Task CheckLaunchOptionDriftAsync()
    {
        try
        {
            ...
            Dispatcher.Invoke(() => toast.ShowAction(
                DepotToolsGui.Resources.Strings.Launch_Drift_Title,
                string.Format(DepotToolsGui.Resources.Strings.Launch_Drift_Body, drifted.Count),
                DepotToolsGui.Resources.Strings.Launch_Drift_Action,
        ...
        Services.AppInfo.LaunchOptionsService launch, IReadOnlyList<int> drifted, ToastService toast)
    {
        ...
        if (MessageBox.Show(
                DepotToolsGui.Resources.Strings.Launch_ApplyNow_Body,
                DepotToolsGui.Resources.Strings.Launch_ApplyNow_Title,
        ...
        if (result.Ok)
            toast.Show(DepotToolsGui.Resources.Strings.Launch_Title,
                result.SteamWasRunning
                    ? DepotToolsGui.Resources.Strings.Launch_Applied_Restarted
                    : DepotToolsGui.Resources.Strings.Launch_Applied);
        else
            toast.Show(DepotToolsGui.Resources.Strings.Launch_Title,
    ...
    protected override async void OnStartup(StartupEventArgs e)
    {
        ...
        // Legacy cleanup: older builds staged downloads in ~/Downloads/DepotTools (they now stage in
        ...
        _ = System.Threading.Tasks.Task.Run(() =>
        {
            try
            {
                string legacy = System.IO.Path.Combine(
                    Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), "Downloads", "DepotTools");
        ...
        settingsVm.RequestRestartPrompt = () => Dispatcher.Invoke(() =>
            toast.ShowAction(
                DepotToolsGui.Resources.Strings.Lang_Changed_Title,
                DepotToolsGui.Resources.Strings.Lang_Changed_Body,
        ...
        // depottools://install/<id> are literally the same path and can't drift apart later.
        // DropInstallViewModel is transient, so Home and Add each hold their own instance.
        Func<long, Task> installByAppId = appId =>
        {
            Dispatcher.Invoke(() => HandleProtocolUrl($"depottools://install/{appId}"));
        ...
        // A silent install launch (depottools://install/silent/<id>) runs headless: stay in the tray and
    ...
    /// <summary>Route a depottools:// protocol URL to the appropriate page and action.</summary>
