...
public partial class SourceRowViewModel : ObservableObject
{
    ...
    /// <summary>Hide the status badge when availability is unknown (e.g. a DepotBox row with no key set.
    ...
    /// <summary>DepotBox key not configured. Show a hint instead of a download button.</summary>
    [ObservableProperty]
...
{
    private readonly DepotBoxService _api;
    ...
    // Set while a depottools://install/silent/<id> install runs so the download path skips the interactive
    // overwrite-confirm overlay (there's no surfaced window for the user to confirm it on).
    private bool _silentInstall;

    /// <summary>
    /// Called by the protocol handler for depottools://install/&lt;appid&gt;. Seeds the app, fetches details,
    ...
    /// FetchAsync pipeline the app UI uses (dynamic sources + DepotBox synth + key-gating + usage +
    ...
    private async Task FetchAsync()
    {
        ...
        if (Details.IsDlc && !HasDepotBoxKey)
        {
            Error = Resources.Strings.Add_NeedsDepotBoxKey;
            return;
        }
        ResetResults();
        IsChecking = true;
    ...
    private const string DepotBoxSourceName = "DepotBox";

    /// <summary>
    /// Inject the DepotBox row into the source-status map. The manifest backend no longer returns it, so
    /// availability comes from a direct DepotBox status check using the user's own key. With no key we still
    ...
    private async Task AddDepotBoxSourceAsync(Dictionary<string, string> statuses, string appid)
    {
        ...
        if (string.IsNullOrEmpty(key))
        {
            ...
            statuses[DepotBoxSourceName] = "unknown";
            return;
        }
    ...
    private async Task ApplyDepotBoxStateAsync()
    {
    ...
    /// <summary>
    /// Refresh the standard "X/25" daily usage badge on every non-DepotBox source row (those count
    /// toward the DepotBox daily limit; DepotBox sources show their own X/800 instead). Signed-in only.
    /// </summary>
    ...
    public async Task DownloadFromSourceAsync(SourceRowViewModel source)
    {
        ...
        // DepotBox downloads use the user's OWN key and never touch DepotBox, so a guest with a key
        bool depotboxWithKey = source.NeedsKey && HasDepotBoxKey;
        if (!depotboxWithKey)
        {
            Error = Resources.Strings.Add_NeedsDepotBoxKey;
            return;
        ...
        try
        {
            ...
            // Key-gated (DepotBox) sources download DIRECTLY from depotboxmanifest.com with the user's key;
            // everything else goes through DepotBox and counts toward the 25/day limit.
            DownloadedFile download;
            if (source.NeedsKey)
            {
                download = await _depotBox.DownloadManifestAsync(
                    appId.ToString(), _settings.DepotBoxApiKey ?? "", progress);
            }
            else
            {
            ...
            if (source.NeedsKey) await ApplyDepotBoxStateAsync(); // refresh the key's X/Y usage badge
            else await RefreshStandardUsageAsync(); // non-DepotBox usage just changed (counts toward 25/day)
    ...
    private static LuaContents? ExtractLuaFromZip(string zipPath, long appId)
    {
        try
        {
            ...
            string tmp = Path.Combine(Path.GetTempPath(), $"depottools_diff_{appId}_{Guid.NewGuid():N}.lua");
