...
public partial class DropInstallViewModel : ObservableObject
{
    ...
    /// <summary>
    /// Install a game by appid. Set by App to the SAME path <c>depottools://install/&lt;appid&gt;</c> takes,
    ...
    private static LuaContents? ExtractLuaFromZip(string zipPath, long appId)
    {
        try
        {
            ...
            string tmp = Path.Combine(Path.GetTempPath(), $"depottools_drop_{appId}_{Guid.NewGuid():N}.lua");
